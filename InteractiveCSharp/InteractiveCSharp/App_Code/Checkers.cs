...
namespace InteractiveCSharp
{
    static class Checkers // возврат 0 - удачно; 1 - неверный ответ; 2 - превышено время ожидания
    {
        ...
        public static byte Check(string problemPath)
        {
            ...
            {
                checkerResult = Checker(File.ReadAllLines(file, System.Text.Encoding.GetEncoding(1251)).ElementAt(0), File.ReadAllLines(Directory.EnumerateFiles(problemPath + "/output/").ToList().ElementAt(files.IndexOf(file)), System.Text.Encoding.GetEncoding(1251)).ElementAt(0));
